...
def update_reserva(reserva_id: int, reserva_update: schemas.ReservaUpdate, db: Session = Depends(get_db)):
    db_reserva = crud_reserva.update_reserva(db=db, reserva_id=reserva_id, reserva_update=reserva_update)
    if db_reserva is None:
